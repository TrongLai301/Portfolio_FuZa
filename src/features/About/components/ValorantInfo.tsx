...
export default function ValorantInfo() {
  ...
  useEffect(() => {
    async function fetchLiveProfile() {
      // Split IGN (e.g. "FuZa#Kyu") to get Name and Tag
    ...
    <div className="animation fade-in-up w-full h-full p-4 md:p-6 overflow-y-auto custom-scrollbar flex flex-col gap-8">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-stretch gap-8 bg-[#1a1d2e] bg-opacity-70 p-8 rounded-4xl border border-gray-700 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
        ...
        <div className="relative shrink-0 flex items-center justify-center">
          <div className="absolute inset-0 bg-[#536976] blur-2xl opacity-40 rounded-full"></div>
          <img 
            src={displayAvatar} 
            alt="Avatar" 
            ...
            {activeTab === "graphics" && (
              <div className="flex flex-col gap-6">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-6 border-t border-gray-700 pt-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-6 border-t border-gray-700 pt-5">
...
const SettingGraphicItem = ({ label, value }: { label: string, value: string }) => (
  <div className="flex flex-col">
    <span className="text-gray-400 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-1">{label}</span>
    ...
    <div className="flex flex-wrap items-center gap-4 p-4 flex-1">
      {agents.map(agent => (
        <div key={agent.id} className="relative group cursor-pointer inline-block z-10 hover:z-50">
          <div className="w-16 h-16 md:w-20 md:h-20 bg-[#1a1d2e] rounded-xl overflow-hidden border-2 border-gray-500 group-hover:border-[#536976] transition-all duration-300 transform group-hover:-translate-y-1 group-hover:scale-110 group-hover:shadow-[0_10px_15px_-3px_rgba(83,105,118,0.6)] relative z-20">
            <img src={agent.img} alt={agent.name} className="w-full h-full object-cover scale-110" />
          </div>
          <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 bg-[#1a1d2e] text-white text-xs font-bold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity z-50 whitespace-nowrap border border-gray-600 shadow-lg pointer-events-none">
            {agent.name}
          </span>
        </div>
      ))}
